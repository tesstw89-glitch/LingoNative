        ...
        if let question = viewModel.currentQuestion,
           question.type == .speaking {
            ...
            if session.completionNodeID != nil &&
                speakingPhraseIsComplete(
                    target: question.phrase.foreign,
...
private func speakingPhraseIsComplete(
    target: String,
    transcript: String
) -> Bool {
    let targetWords = speechWords(from: target)
    var heardWords = speechWords(from: transcript)

    guard !targetWords.isEmpty else { return false }

    for word in targetWords {
        guard let index = heardWords.firstIndex(of: word) else {
            return false
        }

        heardWords.remove(at: index)
    }
